...
async def suggest_lead_creation(update: Update, context: ContextTypes.DEFAULT_TYPE, user_id: int):
    """Suggest creating a lead if user shows interest"""
    keyboard = [
    ...
    await update.message.reply_text(
        "¿Deseas que registre esto como un nuevo lead en nuestro sistema?",
        reply_markup=reply_markup
    )
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if query.data == 'qualify_lead':
        ...
        if user_id not in user_data:
            user_data[user_id] = {}
        user_data[user_id]['stage'] = 'qualifying'
